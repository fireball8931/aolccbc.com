...
function buildProgramPage(programnameasurl, programname, programtype, hide_data_table1) {
    try {
        ...
        fetch(JSONFile).then(function(response) {
            ...
            if (response.status == 404) alert("Error, the " + JSONFile + " cannot be found");
            ...
            if (newMinWageDate < new Date()) bcminwage = 15.65;
            else bcminwage = 15.20;
             //console.log(newMinWageDate) 
            //console.log('BC Minimum Wage is: ' + bcminwage)
            const bcminannualsalary = bcminwage * 2080;
            let salarystart = data.salarystart;
            if (salarystart < bcminannualsalary) salarystart = bcminannualsalary;
            salarystart = toCAD(salarystart, "salararystart");
            //console.log(salarystart);
            salaryend = toCAD(data.salaryend, "salaryend");
            if (!data.prog_video) syllabuslink = "<a href=smorgs/" + data.syllabuslink + ' target="_blank">Click Here</a>';
            else syllabuslink = "<a href=smorgs/" + data.syllabuslink + ' target="_blank">Click Here</a> <iframe width="345" height="194" src=' + data.prog_video + ' frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
            function createDivfromJSON(part, alt_heading) {
                //console.log(partname);
                partcontent = "";
                part_alt_heading = "";
                part.paragraphs.forEach((paragraph)=>{
                    if (paragraph.li_title) partcontent = partcontent + "<b>" + paragraph.li_title + "</b><br />";
                    if (paragraph.ul_start) partcontent = partcontent + "<ul>";
                    else if (paragraph.ul_end) partcontent = partcontent + "</ul>";
                    else partcontent = partcontent + "<" + paragraph.style + ">" + paragraph.content + "</" + paragraph.style + ">";
            ...
            fetch(proglistingJSONFile).then(function(response) {
                ...
                data.programs.forEach((program)=>{
                    ...
                    if (program.name.toLowerCase() === mainname.toLowerCase()) {
                        ...
                        if (workexphours === undefined) //console.log("Whelp");
                        workexphours = "";
                        ...
                        if (coursemat === undefined) coursemat_html = "";
                        else {
                        ...
                        if (program.cooperative_placement_hours === undefined) optional_cooperative_placement_hours = "";
                        else cooperative_placement_hours = program.duration[0].cooperative_placement_hours;
                        // build cell titles
                        ...
                        if (hide_data_table1 == true) // alert(hide_data_table)
                        ...
                        if (workexphours === undefined) document.getElementById("workexphours").innerHTML = "";
                    }
                });
...
//# sourceMappingURL=programs.77692b11.js.map
